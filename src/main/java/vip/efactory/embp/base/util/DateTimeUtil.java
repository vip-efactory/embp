...
public abstract class DateTimeUtil {
    ...
    /**
     * 对日期的字符串解析为时间类型！支持格式:"yyyy/MM/dd";"yyyy_MM_dd";"yyyy-MM-dd";"yyyyMMdd"
     *
     * @param dateYMD
     * @return
     */
    public static Date getDateFromString(String dateYMD) {
        if (!CommUtil.isEmptyString(dateYMD)) {
            String pattern;
            if (dateYMD.indexOf("/") > 0) {
                pattern = "yyyy/MM/dd";
            } else if (dateYMD.indexOf("_") > 0) {
                pattern = "yyyy_MM_dd";
